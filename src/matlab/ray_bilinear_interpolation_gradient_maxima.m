...
Df_t = collect( simplify( subs(Df, [x y], [x_t, y_t]) ), t);
...
% Partial derivatives
Dvx = simplify(diff(v_xy, x));
Dvy = simplify(diff(v_xy, y));

% Directional derivative
Dv = Dvx*dx + Dvy*dy;

Dv_t = collect( simplify( subs(Dv, [x y], [x_t, y_t]) ), t);
...
c0 = simplify( f_t_coeffs(3) )
c1 = simplify( f_t_coeffs(2) )
c2 = simplify( f_t_coeffs(1) );
...
% Differences
Bf10 = collect(simplify(Bf1 - Bf0), [dx, dy]);
Bf20 = collect(simplify(Bf2 - Bf0), [dx, dy]);
...
%% --------------------------------------------------------------------
cv0 = simplify( v_t_coeffs(3) );
cv1 = simplify( v_t_coeffs(2) );
cv2 = simplify( v_t_coeffs(1) );
...
Bv10 = collect(simplify(Bv1 - Bv0), [dx, dy]);
Bv20 = collect(simplify(Bv2 - Bv0), [dx, dy]);
...
Df_t_B0 = simplify(Df_t_coeffs(2));
Df_t_B1 = simplify(Df_t_coeffs(2) + Df_t_coeffs(1));
...
% We have ax = 0
maxima = [
    simplify(subs( Df_t_B0, [ax,ay,bx,by], [0,0, 1,0])), ...
    simplify(subs( Df_t_B0, [ax,ay,bx,by], [0,0, 1,1])), ...
    simplify(subs( Df_t_B0, [ax,ay,bx,by], [0,1, 1,1])), ...
    simplify(subs( Df_t_B1, [ax,ay,bx,by], [0,0, 1,0])), ...
    simplify(subs( Df_t_B1, [ax,ay,bx,by], [0,0, 1,1])), ...
    simplify(subs( Df_t_B1, [ax,ay,bx,by], [0,1, 1,1])), ...
];
