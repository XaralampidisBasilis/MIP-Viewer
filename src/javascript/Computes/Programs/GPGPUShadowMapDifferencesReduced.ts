...
class UnidirectionalVertexDifferences implements GPGPUProgram 
{
    variableNames = ['A', 'B']
    outputShape: number[]
    userCode: string
    ...
    constructor(
        ...
        void main()
        {
            ...
            float v111 = getBAt(coords + ivec3(${transformOffset(-0,-0,-0)}));
...
class PropagateUnidirectionalVertexDifferences implements GPGPUProgram 
{
    variableNames = ['A', 'B']
    outputShape: number[]
    userCode: string
...
class PropagateUnidirectionalVertexDifferencesGates implements GPGPUProgram 
{
    variableNames = ['A', 'B', 'C']
    outputShape: number[]
    userCode: string
...
function unidirectionalDifferencesGates(
    [minVolume, maxVolume]: [tf.Tensor3D, tf.Tensor3D], 
    gates: tf.Tensor5D,
    permute: Permute, 
    reverse: Reverse,
...
): tf.Tensor5D
{
    ...
    const program = new UnidirectionalVertexDifferences(minVolume.shape, permute, reverse)
    let differences = runWebGLProgram(program, [minVolume, maxVolume], 'float32', [], true) as tf.Tensor5D
    ...
    const propagate = new PropagateUnidirectionalVertexDifferencesGates(shape, permute, reverse)
...
): tf.Tensor5D
{
    ...
    const shape = slices[0].shape as [number, number, number, 2, 2]
    const propagate = new PropagateUnidirectionalVertexDifferences(shape, permute, reverse)
...
) : tf.Tensor3D
{
    ...
    const differences = unidirectionalDifferences([minVolume, maxVolume], permute, reverse) as tf.Tensor5D
    if (verbose) logTensor('differencesPropagated', differences)
...
) : tf.Tensor3D
{
    ...
    const forwardShadowMap = computeUnidirectionalShadowMap([minVolume, maxVolume], dominantAxis, octant, tolerance)
    ...
    const backwardDifferences = unidirectionalDifferencesGates([minVolume, maxVolume], backwardGates, permute, backwardReverse)
...
{
    const shadowMap = computeBidirectionalShadowMap([minVolume, maxVolume], dominantAxis, reverseOctant(octant), tolerance, verbose)
    return shadowMap as tf.Tensor3D
}
...
) : tf.Tensor3D
{
    ...
    // dominantAxis = x
    anisotropicMaps = []
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'x', '+++', tolerance))
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'x', '+-+', tolerance))
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'x', '++-', tolerance))
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'x', '+--', tolerance))
    ...
    // dominantAxis = y
    anisotropicMaps = []
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'y', '+++', tolerance))
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'y', '-++', tolerance))
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'y', '++-', tolerance))
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'y', '-+-', tolerance))
    ...
    // dominantAxis = z
    anisotropicMaps = []
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'z', '+++', tolerance))
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'z', '+-+', tolerance))
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'z', '-++', tolerance))
    anisotropicMaps.push(computeUnidirectionalShadowMap([minVolume, maxVolume], 'z', '--+', tolerance))
...
) : tf.Tensor3D
{
    ...
    anisotropicMaps = []
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'x', '+++', tolerance))
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'x', '+-+', tolerance))
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'x', '++-', tolerance))
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'x', '+--', tolerance))
    ...
    anisotropicMaps = []
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'y', '+++', tolerance))
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'y', '-++', tolerance))
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'y', '++-', tolerance))
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'y', '-+-', tolerance))
    ...
    anisotropicMaps = []
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'z', '+++', tolerance))
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'z', '+-+', tolerance))
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'z', '-++', tolerance))
    anisotropicMaps.push(computeBidirectionalShadowMap([minVolume, maxVolume], 'z', '--+', tolerance))
...
{
    const tempMap = computeUnidirectionalShadowMap([minVolume, maxVolume], 'z', '+++', tolerance)
