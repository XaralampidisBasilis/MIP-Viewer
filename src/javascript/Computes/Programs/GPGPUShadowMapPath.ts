...
class PropagateUnidirectionalMinima implements GPGPUProgram 
{
    variableNames = ['A', 'B']
    outputShape: number[]
    userCode: string
    ...
    constructor(
        ...
        void main()
        {
            ...
            float v110 = getBAt(coords + ivec3(${transformOffset(-0,-0,-1)}));
            float v100 = getBAt(coords + ivec3(${transformOffset(-0,-1,-1)}));
            float v010 = getBAt(coords + ivec3(${transformOffset(-1,-0,-1)}));
            float v000 = getBAt(coords + ivec3(${transformOffset(-1,-1,-1)}));

            v111 = max(v111, min4(v000, v010, v100, v110));
...
class UnidirectionalDifferences implements GPGPUProgram 
{
    ...
    constructor(
        ...
        void main()
        {
            ...
            float a111 = getAAt(coords + ivec3(${transformOffset(-0,-0,-0)}));
            float b110 = getBAt(coords + ivec3(${transformOffset(-0,-0,-1)}));
            float b100 = getBAt(coords + ivec3(${transformOffset(-0,-1,-1)}));
            float b010 = getBAt(coords + ivec3(${transformOffset(-1,-0,-1)}));
            float b000 = getBAt(coords + ivec3(${transformOffset(-1,-1,-1)}));

            float d000 = b000 - a111;
            float d010 = b010 - a111;
            float d100 = b100 - a111;
            float d110 = b110 - a111;
...
): tf.Tensor5D
{
    ...
    if (verbose) logTensor('volume', volume)

    const shape = slices[0].shape as [number, number, number, 2, 2]
    const propagate = new PropagateUnidirectionalMinima(shape, permute, reverse)
    ...
    const minima = stack3d(slices, axis) as tf.Tensor5D 
    tf.dispose(slices)
    if (verbose) logTensor('minima', minima)

    const program = new UnidirectionalDifferences(volume.shape, permute, reverse)
    let differences = runWebGLProgram(program, [volume, minima], 'float32', [], true) as tf.Tensor5D
    tf.dispose(minima)

    if (verbose) logTensor('differences', differences)
...
{
    const tempMap = computeUnidirectionalShadowMap(volume, [0,1,2], [ ], tolerance, verbose)
