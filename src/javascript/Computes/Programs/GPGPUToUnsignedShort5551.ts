...
class RGBAToUnsignedShort5551 implements GPGPUProgram 
{
    ...
    constructor(inputShape: number[]) 
    {
        ...
        void uint5551ToRgba(uint u, out uint r, out uint g, out uint b, out uint a)
        {
            r = (u >> 11) & 31u;  // 5 bits
            g = (u >>  6) & 31u;  // 5 bits
            b = (u >>  1) & 31u;  // 5 bits
...
export function packUnsignedShort5551(inputR: tf.Tensor, inputG: tf.Tensor, inputB: tf.Tensor, inputA: tf.Tensor): tf.Tensor
{
    ...
    const program = new RGBAToUnsignedShort5551(shape)
