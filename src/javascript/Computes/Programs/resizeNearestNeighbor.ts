...
export class ResizeNearestNeighborProgram implements GPGPUProgram 
{
    ...
    ) 
    {
        ...
        void main() 
        {
            ...
            ivec3 nearestIndex = clamp(ivec3(floor(sourceFracIndex)), ivec3(0), inputShape - 1);

            float value = getInput(nearestIndex.x,  nearestIndex.y,  nearestIndex.z);
...
{
    const program = new ResizeNearestNeighborProgram(inputTensor.shape, outputShape, alignCorners, halfPixelCenters);
