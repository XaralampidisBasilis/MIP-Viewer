...
class UnidirectionalFaceMinimaMapHollow implements GPGPUProgram 
{
    variableNames = ['A', 'B']
    outputShape: number[]
    userCode: string
...
class UnidirectionalFaceMinimaMap implements GPGPUProgram 
{
    variableNames = ['A']
    outputShape: number[]
    userCode: string
...
class UnidirectionalFaceMaximaMap implements GPGPUProgram 
{
    variableNames = ['A']
    outputShape: number[]
    userCode: string
...
class PropagateUnidirectionalFaceMinimaSlices implements GPGPUProgram 
{
    variableNames = ['A', 'B']
    outputShape: number[]
    userCode: string
...
class PropagateUnidirectionalFaceMinimaMap implements GPGPUProgram 
{
    variableNames = ['A']
    outputShape: number[]
    userCode: string
...
): tf.Tensor5D
{
    ...
    const program = new UnidirectionalFaceMinimaMapHollow(volume.shape, permute, reverse)
    ...
    const propagate = new PropagateUnidirectionalFaceMinimaSlices(shape, permute, reverse)
...
): tf.Tensor5D
{
    ...
    const program = new UnidirectionalFaceMinimaMap(volume.shape, permute, reverse)
...
{
    const program = new UnidirectionalFaceMaximaMap(volume.shape, permute, reverse)
...
) : tf.Tensor3D
{
    ...
    const minimaMap = unidirectionalMinimaMap(minVolume, permute, reverse)
    if (verbose) logTensor('minimaMap', minimaMap)

    const maximaMap = unidirectionalMaximaMap(maxVolume, permute, reverse)
...
) : tf.Tensor3D
{
    ...
    const backwardMinimaMap = unidirectionalMinimaMapHollow(minVolume, forwardShadowMap, permute, backwardReverse)
    const backwardMaximaMap = unidirectionalMaximaMap(maxVolume, permute, backwardReverse)
