...
function computeComparisonBidirectionalBlockShadowMap(
    volume: tf.Tensor3D,
    dominantAxis: Axis,
    octant: Octant,
    tolerance: number,
...
{
    const shadowMapA = computeBidirectionalBlockShadowMapA(volume, dominantAxis, octant, tolerance, blockSize, false)
    const shadowMapB = computeBidirectionalBlockShadowMapB(volume, dominantAxis, octant, tolerance, blockSize, false)

    if (verbose)
    {
        logTensor('shadowMapA', shadowMapA)
        logTensor('shadowMapB', shadowMapB)
    }

    const shadowMapC = tf.tidy(() => tf.maximum(tf.sub(shadowMapB, shadowMapA), 0)) as tf.Tensor3D
    if (verbose) logTensor('shadowMapC', shadowMapC)

    tf.dispose([shadowMapA, shadowMapB])
...
{
    const shadows = computeComparisonBidirectionalBlockShadowMap(volume, dominantAxis, octant, tolerance, blockSize, false)
