...
class ExtendedAnisotropicChebyshevDistancePassZ implements GPGPUProgram 
{
    variableNames = ['A']
    outputShape: number[]
    userCode: string
    ...
    constructor(inputShape: [number, number, number]) 
    {
        ...
        this.userCode = `
        const int MAX_DISTANCE = 31;

        ivec3 getCoords()
        {
            return getOutputCoords().zyx;
        }

        float getA(ivec3 coords) 
        { 
            return getA(coords.zyx); 
        }

        int getB(ivec3 coords) 
        {
            return (getA(coords) < 0.5) ?  MAX_DISTANCE : 0;
        }
 
        void main() 
        {
            ivec3 pos0 = getCoords()
            ivec3 pos = pos0;

            int outDistance = getB(pos);
            if (outDistance <= 1) 
            {
                setOutput(float(outDistance));
                return;
            }
            ...
            for (int step = 1; step <= MAX_DISTANCE; step++) 
            {
                ...
                if (pos.z >= 0) 
                {
                    int inDistance = getB(pos);
                    outDistance = min(outDistance, max(inDistance, step));

                    if (outDistance <= step) break;
                }
...
class ExtendedAnisotropicChebyshevDistancePassY implements GPGPUProgram 
{
    variableNames = ['A']
    outputShape: number[]
    userCode: string
