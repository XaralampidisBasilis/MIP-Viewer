...
export async function min3dWebGPU(input, options = {})
{
    const { awaitCompletion = false } = options

    if (input.dtype !== 'float32')
    {
        throw new Error(`min3dWebGPU only supports float32 tensors, got "${input.dtype}".`)
    }

    if (input.size <= 0)
    {
        throw new Error('min3dWebGPU expected a non-empty tensor.')
    }
    ...
    while (inputCount > 1)
    {
        // Each output value is the minimum of up to REDUCE_WORKGROUP_SIZE values.
        ...
        await runComputeProgram(input.device, {
            label: 'reduce-min',
            code: min3dWGSL(inputCount, outputCount, dispatch[0]),
            bindings: [
                { buffer: inputBuffer },
                { buffer: outputBuffer },
            ],
...
function reduceDispatch(device, outputCount)
{
    ...
    {
        throw new Error(
            `min3dWebGPU needs dispatch [${x}, ${y}, 1], but this WebGPU device allows only ${maxWorkgroupsPerDimension} workgroups per dimension.`,
        )
    }
...
function min3dWGSL(inputCount, outputCount, dispatchX)
{
    return /* wgsl */ `
    ...
    // Used as the neutral value for min reduction.
    const POS_INF: f32 = 3.402823466e+38;

    // Raw input values to reduce.
    @group(0) @binding(0) var<storage, read> input_values: array<f32>;

    // One output minimum value per workgroup.
    ...
    )
    {
        ...
        // Out-of-bounds lanes use POS_INF so they do not affect the minimum.
        var value = POS_INF;
        ...
        if (input_index < INPUT_COUNT)
        {
            ...
            value = select(raw, POS_INF, raw != raw);
        }
        ...
        // The first invocation writes the workgroup's final minimum.
        if (local_index == 0u)
        {
            output_values[output_index] = mins[0];
        }
    }
`
}
