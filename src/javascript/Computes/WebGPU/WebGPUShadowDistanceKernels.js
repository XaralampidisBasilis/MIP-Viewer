...
function computeCellShadowsWGSL( inputShape, outputShape, axis, octant ) {
	...
	{
	    return vertex_shadows[index3(clamp3(coords))];
	}
	...
	fn main(@builtin(global_invocation_id) gid: vec3<u32>)
	{
	    ...
	    cell_shadows[out_index(coords)] = shadow;
	}
`;
}
	    ...
	    for (var dz = 0; dz < BLOCK_SIZE; dz = dz + 1) {
	        for (var dy = 0; dy < BLOCK_SIZE; dy = dy + 1) {
	            for (var dx = 0; dx < BLOCK_SIZE; dx = dx + 1) {
	                ...
	                value = value & input_values[index3(coords)];
	            }
	        }
	    }
...
function anisotropicChebyshevDistanceWGSL( shape, axis, sign, maxDistance ) {
	...
	fn main(@builtin(global_invocation_id) gid: vec3<u32>)
	{
	    ...
	    let out_coords = vec3<i32>(i32(gid.x), i32(gid.y), i32(gid.z));
	    let out_index_value = index3(out_coords);
