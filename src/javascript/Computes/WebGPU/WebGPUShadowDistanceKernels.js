...
export async function computeBidirectionalShadowMapWebGPU(
    volume,
    dominantAxis,
    directionOctant,
    tolerance,
...
)
{
    ...
    const forwardVertexMinmax = await computeVertexValues(volume)
    await propagateVertexMinmaxValuesInPlace(forwardVertexMinmax, dominantAxis, forwardOctant)
    const forwardVertexShadows = await computeVertexShadowsFromVolume(volume, forwardVertexMinmax, dominantAxis, forwardOctant, tolerance)
    forwardVertexMinmax.dispose()

    const forwardCellShadows = await computeCellShadows(forwardVertexShadows, dominantAxis, forwardOctant)
    forwardVertexShadows.dispose()

    const backwardOctant = su.reverseOctant(forwardOctant)
    const backwardVertexMinmax = await computeVertexValuesWithHoles(volume, forwardCellShadows, dominantAxis, backwardOctant)
    await propagateVertexMinmaxValuesInPlace(backwardVertexMinmax, dominantAxis, backwardOctant)
    const backwardVertexShadows = await computeVertexShadowsFromVolumeHoles(volume, forwardCellShadows, backwardVertexMinmax, dominantAxis, backwardOctant, tolerance)
    backwardVertexMinmax.dispose()

    const backwardCellShadows = await computeCellShadows(backwardVertexShadows, dominantAxis, backwardOctant)
...
async function computeVertexValues(volume)
{
    ...
    const output = WebGPUTensor3D.empty(volume.device, [depth + 2, height + 2, width + 2], 'float32', 'webgpu-vertex-values')
    ...
    await runComputeProgram(volume.device, {
        ...
        dispatch: dispatchForShape(output.shape, WORKGROUP_SIZE),
    })
...
{
    const [depth, height, width] = volume.shape
    const output = WebGPUTensor3D.empty(volume.device, [depth + 2, height + 2, width + 2], 'float32', 'webgpu-vertex-values-holes')
...
async function computeVertexShadowsFromVolumeHoles(volume, cellShadows, vertexMinmax, axis, octant, tolerance)
{
    ...
    await runComputeProgram(volume.device, {
        ...
        code: computeVertexShadowsFromVolumeHolesWGSL(volume.shape, cellShadows.shape, vertexMinmax.shape, axis, octant, tolerance),
        bindings: [
            { buffer: volume.buffer },
            { buffer: cellShadows.buffer },
            { buffer: vertexMinmax.buffer },
...
async function computeCellShadows(vertexShadows, axis, octant)
{
    ...
    const output = WebGPUTensor3D.empty(vertexShadows.device, [depth - 1, height - 1, width - 1], 'uint32', 'webgpu-cell-shadows')
...
async function logicalOrInPlace(a, b)
{
    await runComputeProgram(a.device, {
        ...
        dispatch: dispatchForShape(a.shape, WORKGROUP_SIZE),
    })
...
async function initialChebyshevDistanceInPlace(shadowMap, maxDistance)
{
    await runComputeProgram(shadowMap.device, {
        ...
        dispatch: dispatchForShape(shadowMap.shape, WORKGROUP_SIZE),
    })
...
fn ${prefix.toLowerCase()}_index(coords: vec3<i32>) -> u32 {
    return u32(coords.z) * ${prefix}_HEIGHT * ${prefix}_WIDTH + u32(coords.y) * ${prefix}_WIDTH + u32(coords.x);
}

fn ${prefix.toLowerCase()}_in_bounds(gid: vec3<u32>) -> bool {
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    vertex_values[out_index(vertex_coords)] = volume_at(voxel_coords);
}
`
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let vertex_coords = vec3<i32>(i32(gid.x), i32(gid.y), i32(gid.z));
    vertex_values[out_index(vertex_coords)] = vertex_value_at(vertex_coords);
}
`
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    vertex_shadows[index3(coords)] = select(0u, 1u, margin <= TOLERANCE);
}
`
}
...
fn vertex_shadow_at(coords: vec3<i32>) -> bool {
    return vertex_shadows[index3(clamp3(coords))] != 0u;
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    cell_shadows[out_index(coords)] = select(0u, 1u, shadow);
}
`
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    if (gid.x >= WIDTH || gid.y >= HEIGHT || gid.z >= DEPTH) { return; }
    let idx = index3(vec3<i32>(i32(gid.x), i32(gid.y), i32(gid.z)));
    a_values[idx] = select(0u, 1u, (a_values[idx] != 0u) || (b_values[idx] != 0u));
}
`
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var dz = 0; dz < BLOCK_SIZE; dz = dz + 1) {
        for (var dy = 0; dy < BLOCK_SIZE; dy = dy + 1) {
            for (var dx = 0; dx < BLOCK_SIZE; dx = dx + 1) {
                let coords = start + vec3<i32>(dx, dy, dz);
                if (!valid3(coords)) { continue; }
                found = true;
                value = min(value, input_values[index3(coords)]);
            }
        }
    }

    output_values[out_index(out_coords)] = select(0u, value, found);
}
`
}
...
fn outside_axis(coords: vec3<i32>) -> bool {
    return coords.${axisField} < 0 || coords.${axisField} >= i32(${axisSize(shape, axis)}u);
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let out_coords = vec3<i32>(i32(gid.x), i32(gid.y), i32(gid.z));
    var sample_coords = out_coords;
    var min_distance = input_distances[index3(sample_coords)];

    if (min_distance == 0u) {
        output_distances[index3(out_coords)] = min_distance;
        return;
    }

    for (var radius = 1; radius <= MAX_RADIUS; radius = radius + 1) {
        sample_coords.${axisField} = out_coords.${axisField} ${signOperator} radius;

        if (outside_axis(sample_coords)) {
            break;
        }

        let sample_distance = input_distances[index3(sample_coords)];
