...
export async function minPool3dWebGPU( input, blockSize, options = {} ) 
{
	const {
		disposeInput = false,
	} = options
	...
	if ( input.dtype !== 'float32' && input.dtype !== 'uint32' && input.dtype !== 'int32' ) {
		throw new Error( `minPool3dWebGPU only supports float32, uint32, and int32 tensors, got "${input.dtype}".` )
	}

	if ( input.size <= 0 ) {
		throw new Error( 'minPool3dWebGPU expected a non-empty tensor.' )
	}

	if ( !Number.isInteger( blockSize ) || blockSize <= 0 ) {
		throw new Error( `minPool3dWebGPU expected a positive integer blockSize, got ${blockSize}.` )
	}
	...
	await runComputeProgram( input.device, {
		label: `min-pool3d-${input.dtype}`,
		code: minPool3dWGSL( input.shape, outputShape, blockSize, input.dtype, WORKGROUP_SIZE_3D ),
		bindings: [
			{ buffer: input.buffer },
...
function minPool3dWGSL( inputShape, outputShape, blockSize, dtype, workgroupSize ) 
{
	...
	return /* wgsl */ `
        ...
        const NEUTRAL_MIN: ${dtypeInfo.type} = ${dtypeInfo.neutralMin};
