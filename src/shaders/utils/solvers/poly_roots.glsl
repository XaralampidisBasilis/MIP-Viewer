    ...
    float error_tolerance)
{
    ...
    out_end_value = poly[POLY_MAX_DEGREE];
    #pragma unroll
    for (int i = POLY_MAX_DEGREE - 1; i != -1; --i)
    {
    ...
    for (int i = 0; i != 90; ++i) 
    {
        ...
        float value = poly[POLY_MAX_DEGREE] * current + poly[POLY_MAX_DEGREE - 1];
        float derivative = poly[POLY_MAX_DEGREE];

        #pragma unroll
        for (int j = POLY_MAX_DEGREE - 2; j != -1; --j) 
        {
...
void poly_roots(
    out float out_roots[POLY_MAX_DEGREE + 1], 
    float poly[POLY_MAX_DEGREE + 1], 
    float begin, 
    float end) 
{
    ...
    float derivative[POLY_MAX_DEGREE + 1];
    derivative[0] = poly[POLY_MAX_DEGREE - 2];
    derivative[1] = float(POLY_MAX_DEGREE - 1) * poly[POLY_MAX_DEGREE - 1];
    derivative[2] = (0.5 * float((POLY_MAX_DEGREE - 1) * POLY_MAX_DEGREE)) * poly[POLY_MAX_DEGREE - 0];

    #pragma unroll
    for (int i = 3; i != POLY_MAX_DEGREE + 1; ++i)
    {
        derivative[i] = 0.0;
    }
    ...
    if (discriminant >= 0.0) 
    {
        ...
        out_roots[POLY_MAX_DEGREE - 2] = min(root_0, root_1);
        out_roots[POLY_MAX_DEGREE - 1] = max(root_0, root_1);
    }
    else 
    {
        // Indicate that the cubic derivative has a single root
        out_roots[POLY_MAX_DEGREE - 2] = begin;
        out_roots[POLY_MAX_DEGREE - 1] = begin;
    }
    ...
    out_roots[POLY_MAX_DEGREE] = end;

    #pragma unroll
    for (int i = 0; i != POLY_MAX_DEGREE - 2; ++i)
    {
        out_roots[i] = begin;
    }
    ...
    // treats the derivative as though it had degree POLY_MAX_DEGREE and it
    ...
    #pragma no_unroll
    for (int degree = 3; degree != POLY_MAX_DEGREE + 1; ++degree) 
    {
        ...
        float prev_derivative_order = float(POLY_MAX_DEGREE + 1 - degree);
        #pragma unroll
        for (int i = POLY_MAX_DEGREE; i != 0; --i)
        {
        ...
        for (int i = 0; i != POLY_MAX_DEGREE - 2; ++i)
        {
            derivative[0] = (degree == POLY_MAX_DEGREE - i) ? poly[i] : derivative[0];
        }

        // Determine the value of this derivative at begin
        float begin_value = derivative[POLY_MAX_DEGREE];
        #pragma unroll
        for (int i = POLY_MAX_DEGREE - 1; i != -1; --i)
        {
        ...
        for (int i = 0; i != POLY_MAX_DEGREE; ++i) 
        {
            ...
            else if (degree < POLY_MAX_DEGREE)
            {
    ...
    out_roots[POLY_MAX_DEGREE] = POLY_NO_INTERSECTION;
}
