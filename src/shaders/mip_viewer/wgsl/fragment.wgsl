...
) -> vec4<f32>
{
    ...
    var color = colormap(shading_colormap, mip.value);

    if (debug_option == 101) {
        color = ray.direction * 0.5 + vec3<f32>(0.5);
    } else if (debug_option == 451) {
        color = clamp(mip.position * volume_inv_dimensions, vec3<f32>(0.0), vec3<f32>(1.0));
    } else if (debug_option == 452) {
        color = vec3<f32>(clamp(map_range(box_min_distance, box_max_distance, mip.distance), 0.0, 1.0));
    } else if (debug_option == 601) {
        color = vec3<f32>(clamp(map_range(box_min_distance, box_max_distance, ray.start_distance), 0.0, 1.0));
    } else if (debug_option == 602) {
        color = vec3<f32>(clamp(map_range(box_min_distance, box_max_distance, ray.end_distance), 0.0, 1.0));
    } else if (debug_option == 603) {
        color = vec3<f32>(clamp(map_range(0.0, max(box_span_distance, 1.0e-4), ray.span_distance), 0.0, 1.0));
    }
...
{
    let uv = (frag_coord + vec2<f32>(0.5)) / resolution;
...
{
    let texture_pos = position * volume_inv_dimensions;
    return textureSampleLevel(volume_map, volume_sampler, texture_pos, 0.0).r;
}
