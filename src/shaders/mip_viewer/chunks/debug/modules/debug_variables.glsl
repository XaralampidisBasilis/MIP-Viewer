

// debug.variable0 = to_color(turbo(ray.start_position.x / float(u_volume.dimensions.x)));
// debug.variable1 = to_color(turbo(ray.start_position.y / float(u_volume.dimensions.y)));
// debug.variable2 = to_color(turbo(ray.start_position.z / float(u_volume.dimensions.z)));


debug.variable0 = to_color(hot(map(0.0, u_debug.variable1, abs(mip.value - ref.value))));
debug.variable1 = to_color(hot(map(0.0, u_debug.variable1, abs(mip.distance - ref.distance) / u_box.span_distance)));
 
// PRINT DEBUG
switch (u_debug.option - 1000)
{ 
    case 0  : fragColor = debug.variable0; break;
    case 1  : fragColor = debug.variable1; break;
    case 2  : fragColor = debug.variable2; break;
    case 3  : fragColor = debug.variable3; break;
    case 4  : fragColor = debug.variable4; break;
    case 5  : fragColor = debug.variable5; break;
    case 6  : fragColor = debug.variable6; break;
    case 7  : fragColor = debug.variable7; break;
    case 8  : fragColor = debug.variable8; break;
    case 9  : fragColor = debug.variable9; break;
}