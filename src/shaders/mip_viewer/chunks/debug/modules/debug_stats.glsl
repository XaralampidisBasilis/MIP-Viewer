...
// num cells
vec4 debug_stats_num_cells = to_color(turboBurn(float(stats.num_cells) / float(MAX_CELLS)));

// num traces
vec4 debug_stats_num_traces = to_color(turboBurn(float(stats.num_traces) / float(MAX_TRACES)));

// num mips
vec4 debug_stats_num_mips = to_color(turboBurn(float(stats.num_mips) / float(MAX_CELLS)));

// num cubics
vec4 debug_stats_num_maxima = to_color(turboBurn(float(stats.num_maxima) / float(MAX_CELLS)));
...
// num_volume_fetches
vec4 debug_stats_num_volume_fetches = to_color(turboBurn(float(stats.num_volume_fetches) / float(MAX_CELLS)));

// num_distance_fetches
vec4 debug_stats_num_distance_fetches = to_color(turboBurn(float(stats.num_distance_fetches) / float(MAX_BLOCKS)));
...
switch (u_debug.option - 900)
{
    ...
    case 4: fragColor = debug_stats_num_maxima;            break;
