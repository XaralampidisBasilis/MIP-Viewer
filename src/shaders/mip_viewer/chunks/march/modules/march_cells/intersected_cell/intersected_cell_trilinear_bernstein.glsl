...
cubic.bernstein_coeffs = cubic.values * cubic_bernstein;
...
if (sign_change(cubic.bernstein_coeffs))
{
    ...
    cell.intersected = sign_change(cubic.values) || cubic_has_root(cubic.coeffs, 0.0, 1.0);

    #elif INTERSECTION_TEST == 1
    cell.intersected = sign_change(cubic.values) || cubic_has_root_sample(cubic.coeffs, 0.0, 1.0);
    #endif
